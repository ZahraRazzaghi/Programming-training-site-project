 ...
 * for which we went to check min required length e.g array('username' => 4, 'email =>12)
 * @return array, containing all errors
 */
 ...
 * in this case 'email' and value is the input entred by the user
 * @return array,containing email error
 */
function check_email($data){
    //initializer an array to store error messages
    $form_errors=array();
    $key='email';

    //check if the key email exist data array
    if(array_key_exists($key,$data)){
        ...
        if($_POST[$key] !=null){
            ...
            //chech if input is a valid email address
...
function show_errors($form_errors_array){
    $errors="<p><ul style='color: #f00;'>";

    //loop through error array and display all times in a lis
